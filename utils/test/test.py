...
def test_model_with_patch_analysis(vgg,model, test_loader, checkpoint_path=None, patch_size=16, stride=16, threshold=0.01, device='cpu'):
    if checkpoint_path:
    ...
    with torch.no_grad():
        for images, _ in test_loader:
            ...
            for i in range(min(len(images), 4)):
                ...
                plt.title("Oluşturulmuş Görüntü")
                plt.axis('off')
                ...
                plt.title("Patch Hata Haritası")
                plt.colorbar()
                plt.axis('off')

            plt.show()

            print(f"Batch Yeniden Oluşturma Hatası: {reconstruction_error.item():.4f}")

            anomaly_percentage = torch.mean(anomaly_masks).item() * 100
            print(f"Anormal Batchler'in Yüzdesi: {anomaly_percentage:.2f}%")
